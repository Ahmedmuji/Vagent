...
def get_technical_data_from_gemini(pdf_path, model_name="gemini-3-flash-preview", chunk_output_dir=None):
    ...
    for chunk_index, current_pdf_path in enumerate(chunk_paths):
        ...
        try:
            ...
            "   - Extract EVERY row in technical/compliance/requirements tables, even when the table is long, continues below the visible area, or spans multiple pages.\n"
            ...
            "   - If a table continues on the next page with the same headers or the same section title, keep adding those rows to the same sheet when possible.\n"
