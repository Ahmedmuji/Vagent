...
const state = {
    ...
    function renderModelOptions(models) {
      ...
      models.forEach(model => {
        ...
        option.textContent = `${model.label} â€” $${model.input_per_1m}/$${model.output_per_1m} per 1M`;
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      setProcessing(true, 'Estimatingâ€¦');
    ...
    costConfirmBtn.addEventListener('click', async () => {
      ...
      setProcessing(true, 'Processingâ€¦');
      ...
      item.innerHTML = `
        <div class="flex min-w-0 items-center gap-3">
          ...
          <div class="min-w-0">
            ...
            <div class="mt-0.5 text-[11.5px] text-steel-500 font-mono">${escapeHtml(date)} Â· ${escapeHtml(size)}</div>
          </div>
        </div>
    ...
    async function fetchRecentDownloads() {
      ...
      try {
        ...
        refreshDownloadsBtn.querySelector('span').textContent = 'Checkingâ€¦';
